...
class SNRLoss(torch.nn.Module):
    ...
    def forward(self, x, s, eps=1e-8):
        ...
        loss = 10 * torch.log10(self.l2norm(s - x) ** 2 + self.tao * self.l2norm(s) ** 2 + eps) - 10 * torch.log10(self.l2norm(s) ** 2 + eps)
        return loss.mean()
...
class STOILevelLoss(torch.nn.Module):
    ...
    def measure_loudness(self, signal, eps=1e-8):
        ...
        z_ave_gated_a = torch.sum(z * idx_a, dim=1, keepdim=True) / (torch.sum(idx_a, dim=1, keepdim=True) + eps)
        gamma_r = -0.691 + 10 * torch.log10(z_ave_gated_a + eps) - 10

        idx_r = torch.where(l > gamma_r, 1, 0)
        idx_a_r = idx_a * idx_r
        z_ave_gated_a_r = torch.sum(z * idx_a_r, dim=1, keepdim=True) / (torch.sum(idx_a_r, dim=1, keepdim=True) + eps)
