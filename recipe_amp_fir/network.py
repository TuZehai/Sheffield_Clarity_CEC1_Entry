...
class ConvTasNet(nn.Module):
    def __init__(self, N_spec, N_spat, L, B, H, P, X, R, C, num_channels, norm_type="cLN", causal=False,
                 mask_nonlinear='relu'):
        """
        Args:
        ...
        # Hyper-parameter
        self.N_spec, self.N_spat, self.L, self.B, self.H, self.P, self.X, self.R, self.C = N_spec, N_spat, L, B, H, P, \
                                                                                           X, R, C
...
class TemporalConvNet(nn.Module):
    def __init__(self, N_spec, N_spat, B, H, P, X, R, C, norm_type="gLN", causal=False,
                 mask_nonlinear='relu'):
        """
        Args:
...
if __name__ == "__main__":
    import toml
    config = toml.load('convtasnet.toml')
    network_config = config['amplification_network_config']
    N_spec, N_spat, L, B, H, P, X, R, C, num_channels, causal, norm_type = network_config['N_spec'], \
                                                                           network_config['N_spat'], \
                                                                           network_config['L'], \
                                                                           network_config['B'], \
                                                                           network_config['H'], \
                                                                           network_config['P'], \
                                                                           ...
                                                                           network_config['R'], \
                                                                           network_config['C'], \
                                                                           network_config['num_channels'], \
                                                                           network_config['causal'], \
                                                                           network_config['norm_type']

    # test Conv-TasNet
    mixture = torch.rand(4, num_channels, 16000).to('cuda')
    conv_tasnet = ConvTasNet(**config['amplification_network_config']).to('cuda')
