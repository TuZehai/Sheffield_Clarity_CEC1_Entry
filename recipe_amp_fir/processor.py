...
class AudiometricFIR(nn.Module):
    def __init__(self, nfir=280, sr=44100):
        ...
        linear_fv = np.linspace(0, nfir, nfir + 1) / nfir * sr // 2  # linear frequency vector
        ...
        self.x2_minus_x1 = torch.tensor(x2_minus_x1, dtype=torch.float32, device=self.device)
        self.x_minus_x1 = torch.tensor(x_minus_x1, dtype=torch.float32, device=self.device)

        self.amp = nn.Parameter(torch.tensor(np.ones_like(aud_fv[1:-1]), dtype=torch.float32, device=self.device))

    def forward(self, x):
        amp = torch.pow(10, torch.abs(self.amp) / 20.)
        ...
        magnitudes = torch.cat([gain, phase], dim=1).unsqueeze(0)
        ...
        fir_filter = torch.cat([impulse_response[:, first_half_start:],
                                impulse_response[:, :second_half_end]], dim=-1).unsqueeze(0)
